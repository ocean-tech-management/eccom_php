...
class AreaMemberVdc extends BaseModel
{
    ...
    public function edit(array $data)
    {
        ...
        //把最大(即level最小的等级)剔除,因为不可以参与此处的判断,要从第二大的开始判断
        $oldMaxLevel = min(array_keys($allLevel));
        unset($allLevel[$oldMaxLevel]);

        $nowMaxLevel = min(array_keys($allLevel));
        $nowMinLevel = max(array_keys($allLevel));
        if ($data['level'] != $nowMaxLevel && $data['level'] != $nowMinLevel) {
            //如果是原来最大的则不需要判断, 中间值需要判断,不能超过最大不能小于最小
            if($data['level'] != $oldMaxLevel){
