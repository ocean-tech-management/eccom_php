...
class PtActivity extends BaseModel
{
    ...
    public function cList(array $sear)
    {
        ...
        }, 'startUserType', 'joinUserType'])->where($map)->field('activity_code,activity_title,activity_cover,activity_desc,type,icon,start_user_type,join_user_type,start_time,end_time,expire_time,group_number,join_number,share_title,share_desc,share_cover,create_time')->when($page, function ($query) use ($page) {
            $query->page($page, $this->pageNumber);
        ...
        if (!empty($list)) {
            ...
            if (!empty($allPtSpu)) {
                ...
                if (!empty($allPtGoodsSn) && !empty($allPt)) {
                    $sku = PtGoodsSku::where(['goods_sn' => $allPtGoodsSn, 'activity_code' => $allPt, 'status' => [1]])->select()->toArray();
                    if (!empty($sku)) {
...
//        $res = $this->validate()->baseUpdate(['activity_code'=>$data['activity_code'],'status'=>[1,2]],$data);
    ...
    public function checkExistOrder(array $data)
    {
        ...
        $ptOrder = PtOrder::where(['pay_status' => $orderStatus, 'activity_code' => $activityCode, 'activity_status' => $activityStatus])->count();
        if (!empty($ptOrder)) {
            return true;
        }
    ...
    {
        return $this->hasMany('PtGoods', 'activity_code', 'activity_code')->where(['status' => 1]);
    }
