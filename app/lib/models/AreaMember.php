...
class AreaMember extends BaseModel
{
    protected $field = ['member_card', 'uid', 'user_phone', 'level', 'link_superior_user', 'status', 'leader_level', 'type', 'team_sales_price', 'upgrade_time', 'demotion_team_sales_price', 'team_chain', 'team_sales_price_offset', 'province', 'city', 'area'];
    ...
    public function list(array $sear = []): array
    {
        ...
        if (!empty($sear['province'])) {
            $map[] = ['province', '=', $sear['province']];
        }
        if (!empty($sear['city'])) {
            $map[] = ['city', '=', $sear['city']];
        }
        ...
        if (!empty($page)) {
            $aTotal = Db::name('area_member')->alias('a')
        ...
        $list = Db::name('area_member')->alias('a')
            ->join('sp_user b', 'a.uid = b.uid', 'left')
            ->join('sp_user c', 'a.link_superior_user = c.uid', 'left')
            ->join('sp_area_member_vdc d', 'a.level = d.level', 'left')
            ->field('a.uid,a.member_card,a.user_phone,a.level,a.link_superior_user,b.name as user_name,b.avatarUrl,a.team_sales_price,b.team_performance,b.old_all_team_performance,b.old_update_team_performance,c.name as link_user_name,c.phone as link_user_phone,c.area_vip_level as link_user_level,a.create_time,a.upgrade_time,a.demotion_team_sales_price,a.team_chain,a.divide_chain,b.total_balance,b.ad_balance,a.team_sales_price_offset,a.province,a.city,a.area,d.name as vip_name')
            ->where($map)
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $key => $value) {
                $allAreaCode[] = $value['province'];
                $allAreaCode[] = $value['city'];
                $allAreaCode[] = $value['area'];
            }
            if (!empty($allAreaCode)) {
                $allAreaCode = array_unique($allAreaCode);
                $allAreaName = City::where(['code' => $allAreaCode, 'status' => 1])->column('name', 'code');
                foreach ($list as $key => $value) {
                    if (!empty($value['province'])) {
                        $list[$key]['province_name'] = $allAreaName[$value['province']] ?? null;
                    }
                    if (!empty($value['city'])) {
                        $list[$key]['city_name'] = $allAreaName[$value['city']] ?? null;
                    }
                    if (!empty($value['area'])) {
                        $list[$key]['area_name'] = $allAreaName[$value['area']] ?? null;
                    }
                }
            }
        }
        if (!empty($sear['needOrderTotal'])) {
            if (!empty($list)) {
                ...
                $directUser = AreaMember::where($memberMap)->field('uid,link_superior_user')->select()->toArray();
    ...
    public function levelMember(array $sear = []): array
    {
        ...
        $list = Db::name('area_member')->alias('a')
            ->join('sp_user b', 'a.uid = b.uid', 'left')
            ->join('sp_area_member_vdc c', 'c.level = a.level', 'left')
    ...
    public function assign(array $data)
    {
        ...
        if ($assignType == 2 && empty($memberInfo)) {
            throw new MemberException(['msg' => '该用户不是区域代理无法取消']);
        }
        if ($assignType == 1) {
            //查找是否区域是否存在代理
            $eMap[] = ['level', '=', $data['level']];
            $eMap[] = ['province', '=', $data['province']];
            $eMap[] = ['status', '=', 1];
            $eMap[] = ['uid', '<>', $data['uid']];
            if (!empty($data['city'] ?? 0)) {
                $eMap[] = ['city', '=', $data['city']];
            }
            if (!empty($data['area'] ?? 0)) {
                $eMap[] = ['area', '=', $data['area']];
            }
            $existAreaMember = self::where($eMap)->count();
            if (!empty($existAreaMember)) {
                throw new MemberException(['msg' => '该区域已存在代理']);
            }
        }


        $userInfo = User::where(['uid' => $uid, 'status' => 1])->findOrEmpty()->toArray();
        $vdcInfo = AreaMemberVdc::where(['level' => $level, 'status' => 1])->findOrEmpty()->toArray();
        ...
        if($assignType == 1){
            ...
            $update['city'] = $data['city'] ?? 0;
            $update['area'] = $data['area'] ?? 0;
            ...
            if (empty($memberInfo)) {
                ...
                $res = self::update($update, ['uid' => $uid, 'status' => 1]);
            }
        }else{
            $update['level'] = 0;
            $update['upgrade_time'] = time();
        ...
        User::update(['area_vip_level' => $update['level']], ['uid' => $uid, 'status' => 1]);
...
//        $sql = "(SELECT u2.uid,u2.user_phone,u2.child_team_code,u2.parent_team,u2.level,u2.type,u2.link_superior_user,u2.status,u1.team_level FROM( SELECT @ids AS p_ids, (SELECT @ids := GROUP_CONCAT(child_team_code) FROM sp_area_member WHERE FIND_IN_SET(parent_team, @ids COLLATE utf8mb4_unicode_ci)) AS c_ids, @l := @l+1 AS team_level FROM sp_area_member, (SELECT @ids := " . "'" . $userChildTeamCode . "'" . ", @l := 0 ) b WHERE @ids IS NOT NULL ) u1 JOIN sp_area_member u2 ON FIND_IN_SET(u2.child_team_code, u1.p_ids COLLATE utf8mb4_unicode_ci) " . $mySelfSql . " order by u1.team_level )";
        ...
        if (!empty($databaseVersion) && is_numeric($databaseVersion) && $databaseVersion > 8016) {
            ...
            $sql = "(SELECT u2.uid,u2.user_phone,u2.child_team_code,u2.parent_team,u2.level,u2.type,u2.link_superior_user,u2.status,u1.team_level,u2.create_time FROM( SELECT @ids AS p_ids, (SELECT @ids := GROUP_CONCAT(uid) FROM sp_area_member, (SELECT @ids := " . "'" . $uid . "'" . ", @l := 0 ) b WHERE FIND_IN_SET(link_superior_user, @ids COLLATE utf8mb4_unicode_ci)) AS c_ids, @l := @l+1 AS team_level FROM sp_area_member WHERE @ids IS NOT NULL ) u1 JOIN sp_area_member u2 ON FIND_IN_SET(u2.uid, u1.p_ids COLLATE utf8mb4_unicode_ci) " . $otherSql . $mySelfSql . " order by u1.team_level)";
        } else {
            //mysql 8.0.16及以下使用这条sql
            $sql = "(SELECT u2.uid,u2.user_phone,u2.child_team_code,u2.parent_team,u2.level,u2.type,u2.link_superior_user,u2.status,u1.team_level,u2.create_time FROM( SELECT @ids AS p_ids, (SELECT @ids := GROUP_CONCAT(uid) FROM sp_area_member WHERE FIND_IN_SET(link_superior_user, @ids COLLATE utf8mb4_unicode_ci)) AS c_ids, @l := @l+1 AS team_level FROM sp_area_member, (SELECT @ids := " . "'" . $uid . "'" . ", @l := 0 ) b WHERE @ids IS NOT NULL ) u1 JOIN sp_area_member u2 ON FIND_IN_SET(u2.uid, u1.p_ids COLLATE utf8mb4_unicode_ci) " . $otherSql . $mySelfSql . " order by u1.team_level)";
        }
        ...
        $list = $aTotal = Db::table($sql . " a")
            ->join('sp_user b', 'a.uid = b.uid', 'left')
            ->join('sp_area_member d', 'b.uid = d.uid', 'left')
    ...
    {
        return $this->hasOne('User', 'uid', 'uid')->bind(['user_name' => 'name', 'user_avatarUrl' => 'avatarUrl', 'vip_level', 'user_team_number' => 'team_number', 'old_all_team_performance', 'old_update_team_performance', 'team_performance','area_vip_level']);
    }

    public function pUser()
    {
        return $this->hasOne('User', 'uid', 'link_superior_user')->field('uid,name,avatarUrl,vip_level,phone,area_vip_level');
    }
    ...
    {
        return $this->hasOne('AreaMemberVdc', 'level', 'level')->bind(['vip_name' => 'name']);
    }

    public function userInfo()
    {
        return $this->hasOne('User', 'uid', 'uid')->bind(['user_name' => 'name', 'user_avatarUrl' => 'avatarUrl', 'area_vip_level' => 'level']);
    }
