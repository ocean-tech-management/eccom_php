...
class ShareholderMemberVdc extends BaseModel
{
    ...
    public function list(array $sear = []): array
    {
        ...
        $list = $this->where($map)->field('name,level,discount,vdc_genre,vdc_type,vdc_one,vdc_two,handing_scale,close_divide,grateful_vdc_one,grateful_vdc_two,create_time,sales_price,demotion_sales_price')->when($page, function ($query) use ($page) {
    ...
    public function edit(array $data)
    {
        ...
        if ($data['level'] != $nowMaxLevel && $data['level'] != $nowMinLevel) {
            ...
            if ($data['level'] == $nowMaxLevel && count($allLevel) > 1) {
            ...
            if ($data['level'] == $nowMinLevel && count($allLevel) > 1) {
                unset($allLevel[$data['level']]);
