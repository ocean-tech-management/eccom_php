...
{
    protected $field = ['member_card', 'uid', 'user_phone', 'level', 'link_superior_user', 'status', 'leader_level', 'team_code', 'child_team_code', 'type', 'parent_team', 'growth_value', 'upgrade_time', 'demotion_growth_value', 'team_chain','coder_remark'];
    ...
    public function list(array $sear = []): array
    {
        ...
        if (!empty($page)) {
            $aTotal = Db::name('member')->alias('a')
                ->join('sp_user b', 'a.uid = b.uid', 'left')
                ->join('sp_user c', 'a.link_superior_user = c.uid', 'left')
                ->where($map)->count();
        ...
        $list = Db::name('member')->alias('a')
            ->join('sp_user b', 'a.uid = b.uid', 'left')
            ->join('sp_user c', 'a.link_superior_user = c.uid', 'left')
            ->field('a.uid,a.member_card,a.user_phone,a.level,a.link_superior_user,b.name as user_name,b.avatarUrl,a.growth_value,b.team_performance,b.old_all_team_performance,b.old_update_team_performance,c.name as link_user_name,c.phone as link_user_phone,c.vip_level as link_user_level,a.create_time,a.upgrade_time,a.demotion_growth_value,a.team_chain,a.divide_chain,b.total_balance,b.ad_balance,b.team_vip_level,b.team_sales_price,b.crowd_balance,( b.ad_balance  + b.team_balance + b.area_balance + b.shareholder_balance) as h5_balance, (b.ppyl_balance + b.divide_balance) as shop_balance,b.ban_crowd_transfer,b.openid,b.share_id')
            ->where($map)
        ...
        if (!empty($list)) {
            ...
            //查询用户授权渠道
            $WxUserList = WxUser::where(['openid' => array_unique(array_column($list, 'openid'))])->field('openid,channel,unionId,app_id')->select()->toArray();
            if (!empty($WxUserList)) {
                foreach ($WxUserList as $key => $value) {
                    $WxUserInfo[$value['openid']] = $value;
                }
                foreach ($list as $key => $value) {
                    $list[$key]['auth_channel'] = null;
                    if (!empty($WxUserInfo[$value['openid']] ?? null)) {
                        $list[$key]['auth_channel'] = $WxUserInfo[$value['openid']]['channel'];
                        $list[$key]['wx_app_id'] = $WxUserInfo[$value['openid']]['app_id'] ?? null;
                    }
                }
            }
            ...
            if (!empty($teamTopUserList)) {
                ...
                $allTeamTopUserInfos = User::where(['uid' => $allTeamUidList, 'status' => [1, 2]])->field('uid,name,phone,avatarUrl,vip_level as level')->select()->toArray();
    ...
    public function levelMember(array $sear = []): array
    {
        ...
        $list = Db::name('member')->alias('a')
            ->join('sp_user b', 'a.uid = b.uid', 'left')
            ->join('sp_member_vdc c', 'c.level = a.level', 'left')
        ...
        if ($aNextLevel['need_team_condition'] == 1) {
            ...
            foreach ($recommend_level as $key => $value) {
                foreach ($recommend_number as $nKey => $nValue) {
                    if ($nKey == $key) {
                        $aims[$allCount]['title'] = '直接招募' . $allLevel[$value];
            ...
            foreach ($train_level as $key => $value) {
                foreach ($train_number as $nKey => $nValue) {
                    if ($nKey == $key) {
                        $aims[$allCount]['title'] = '团队招募' . $allLevel[$value];
                        $aims[$allCount]['aimsNumber'] = $nValue;
...
//        $aims[2]['nowNumber'] = !empty($orderTotalPrice) ? $orderTotalPrice : 0;
        ...
        $tallTeam = [];
        $teamAims = [];
        $aTeamLevel = $aMemberInfo['team_vip_level'];
        $tOrderTeamType = [1 => '直属团队', 2 => '整个团队'];
        ...
        if (!empty($aTeamNextLevel['recommend'] ?? null) || !empty($aTeamNextLevel['recommend_level'] ?? null)) {
            //直推人数
            $tdirectTeam = $teamDivide->getNextDirectLinkUserGroupByLevel($aMemberInfo['uid']);
        }
        if (!empty($aTeamNextLevel['train'] ?? null) || !empty($aTeamNextLevel['train_level'] ?? null)) {
            //团队全部人数
            $tallTeam = $teamDivide->getTeamAllUserGroupByLevel($aMemberInfo['uid']);
        }

        if (!empty($aTeamNextLevel['recommend_member_number'])) {
            $allShopMemberNumber = $teamDivide->getNextDirectLinkUserGroupByLevel($aMemberInfo['uid'], -1);
        }
        $aMemberInfo['allTeamNumber'] = $allTeam['allUser']['count'] ?? 0;

        $aMemberInfo['allTeamSalesPrice'] = $aMemberInfo['team_sales_price'];

        //直推要求
        $teamAimsCount = 0;
        if (!empty(json_decode($aNextLevel['recommend_level'], true))) {
        ...
        if (!empty(json_decode($aNextLevel['train_level'], true))) {
        ...
        $teamAims[$tNumber]['nowNumber'] = !empty($aMemberInfo['allTeamSalesPrice']) ? priceFormat($aMemberInfo['allTeamSalesPrice']) : 0;
        $info['teamAims'] = array_values($teamAims);
...
//        $sql = "(SELECT u2.uid,u2.user_phone,u2.child_team_code,u2.parent_team,u2.level,u2.type,u2.link_superior_user,u2.status,u1.team_level FROM( SELECT @ids AS p_ids, (SELECT @ids := GROUP_CONCAT(child_team_code) FROM sp_member WHERE FIND_IN_SET(parent_team, @ids COLLATE utf8mb4_unicode_ci)) AS c_ids, @l := @l+1 AS team_level FROM sp_member, (SELECT @ids := " . "'" . $userChildTeamCode . "'" . ", @l := 0 ) b WHERE @ids IS NOT NULL ) u1 JOIN sp_member u2 ON FIND_IN_SET(u2.child_team_code, u1.p_ids COLLATE utf8mb4_unicode_ci) " . $mySelfSql . " order by u1.team_level )";
        ...
        $list = $aTotal = Db::table($sql . " a")
            ->join('sp_user b', 'a.uid = b.uid', 'left')
            ->join('member d', 'b.uid = d.uid', 'left')
    ...
    {
        return $this->hasOne('User', 'uid', 'uid')->bind(['user_name' => 'name', 'user_avatarUrl' => 'avatarUrl', 'vip_level', 'user_team_number' => 'team_number', 'old_all_team_performance', 'old_update_team_performance', 'team_performance','team_sales_price','team_vip_level']);
    }

    public function tUser()
    {
        return $this->hasOne('User', 'uid', 'uid')->bind(['user_name' => 'name', 'user_avatarUrl' => 'avatarUrl', 'vip_level', 'user_team_number' => 'team_number', 'old_all_team_performance', 'old_update_team_performance', 'team_performance','team_sales_price','team_vip_level','user_phone'=>'phone']);
    }

    public function pUser()
    {
        return $this->hasOne('User', 'uid', 'link_superior_user')->field('uid,name,avatarUrl,vip_level,phone');
    }
    ...
    {
        return $this->hasOne('MemberVdc', 'level', 'level')->bind(['vip_name' => 'name']);
    }
    ...
    {
        return $this->hasOne('TeamMember', 'uid', 'uid')->bind(['team_vip_level' => 'level']);
    }
