...
class GrowthValue
{
    private $normalUserGrowthValueScale = 0.01;
    private $normalUserShareGrowthValueScale = 1;
    ...
    public function buildOrderGrowthValue(string $orderSn)
    {
        ...
        $userInfo = User::with(['topLink'])->where(['uid' => $orderInfo['uid']])->field('uid,phone,vip_level,link_superior_user,growth_value')->findOrEmpty()->toArray();
        ...
        $customGrowth = false;
        $customGrowthGoods = [];
        $customGrowthTotalGrowthValue = 0;
        $notCustomGrowthGoodsPrice = 0;
        ...
        if ($orderInfo['order_type'] == 3) {
            ...
            //尝试在活动专区查找是否有可以自定义成长值的SKU
            $allOrderGoods = OrderGoods::where(['order_sn' => $orderInfo['order_sn'], 'pay_status' => 2, 'status' => 1])->select()->toArray();
            if (!empty($allOrderGoods)) {
                foreach ($allOrderGoods as $key => $value) {
                    $allOrderGoodsInfo[$value['sku_sn']] = $value;
                }
                $orderGoodsGrowthValueList = ActivityGoodsSku::with(['activity'])->where(['sku_sn' => array_column($allOrderGoods, 'sku_sn'), 'status' => 1, 'custom_growth' => 1])->select()->toArray();
                if (!empty($orderGoodsGrowthValueList)) {
                    foreach ($orderGoodsGrowthValueList as $key => $value) {
                        if (!empty($value['growth_value']) && $value['custom_growth'] == 1 && !empty($value['activity']['allow_custom_growth'] ?? null) && $value['activity']['allow_custom_growth'] == 1) {
                            $customGrowthGoods[] = $value['sku_sn'];
                            $customGrowthTotalGrowthValue += ($value['growth_value'] * ($allOrderGoodsInfo[$value['sku_sn']]['count'] ?? 1));
                            $customGrowth = true;
                        }
                    }
                    if (!empty($customGrowthGoods)) {
                        foreach ($allOrderGoods as $key => $value) {
                            if (!in_array($value['sku_sn'], $customGrowthGoods)) {
                                $notCustomGrowthGoodsPrice += ($value['real_pay_price'] - ($value['total_fare_price'] ?? 0));
                            }
                        }
                    }
                }
        ...
        if ($orderPrice >= 1) {
            ...
            if (!empty($orderGoodsGrowthValue['growth_value'])) {
                ...
                if (!empty($userInfo['topLink'])) {
                    //普通用户只能享受实付金额的成长值
                    if(empty(intval($userInfo['topLink']['vip_level']))){
...
//                }
                ...
                //如果存在部分商品有指定成长值则使用指定成长值+部分商品实际销售金额来计算实际得到的成长值
                if(!empty($customGrowth ?? false)){
                    $orderUserGrowth = $customGrowthTotalGrowthValue + priceFormat(($notCustomGrowthGoodsPrice ?? 0) * $vdc[$userInfo['vip_level']]['growth_scale']);
                }else{
...
//                    $orderTopUserGrowth = priceFormat($orderPrice * ($vdc[$userInfo['vip_level']]['growth_scale'] * $vdc[$userInfo['topLink']['vip_level']]['share_growth_scale']));
                    //普通用户只能享受实付金额的成长值
                    if (empty(intval($userInfo['topLink']['vip_level'] ?? 0)) && !empty($customGrowth ?? false)) {
                        $orderTopUserGrowth = priceFormat($orderPrice * ($vdc[$userInfo['vip_level']]['growth_scale'] * $vdc[$userInfo['topLink']['vip_level']]['share_growth_scale']));
                    } else {
                        $orderTopUserGrowth = priceFormat($orderUserGrowth * $vdc[$userInfo['topLink']['vip_level']]['share_growth_scale']);
                    }
                }
            ...
            if (!empty($detail)) {
                ...
                if (!empty($detail[1]) && !empty($userInfo['topLink'] ?? []) && $userInfo['topLink']['vip_level'] == 0) {
                    ...
                    if ((string)$userGrowthValue >= (string)$memberVdc) {
                        ...
                        $topUserOrderInfo['user_phone'] = $userInfo['topLink']['phone'] ?? null;
                        $topUserOrderInfo['link_superior_user'] = $userInfo['topLink']['link_superior_user'] ?? null;
                        $topUserOrderInfo['growth_value'] = $detail[1]['growth_value'] ?? 0;
                        (new MemberService())->becomeMember($topUserOrderInfo);
                    }
                }
            }
