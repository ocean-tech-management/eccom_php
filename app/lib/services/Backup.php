...
// |[ 文档说明: 数据库备份还原类]
// +----------------------------------------------------------------------
// | Copyright (c) 2018~2024 https://www.mlhcmk.com All rights reserved.
// +----------------------------------------------------------------------
...
class Backup
{
    ...
    {
        $config['path'] = app()->getRootPath() . 'public' . DIRECTORY_SEPARATOR . 'static' . DIRECTORY_SEPARATOR . 'data/'; //文件保存默认目录
    ...
    private function get_dbdata($table = '')
    {
        ...
        $columnsDefault = array_column($list, 4);
        foreach ($columnsDefault as $key => $value) {
            if ($value === null) $columnsDefault[$key] = 'NULL';
        }
