...
class TeamMember
{
    ...
    public $memberUpgradeOrderKey = 'teamMemberUpgradeOrder';
    ...
    public function memberUpgrade(string $uid, bool $topUserUpgrade = true)
    {
        ...
        //非会员无法升级
        if (empty($aMemberInfo) || empty($aUserLevel) || empty($aUserInfo) || (!empty($aUserInfo) && (empty($aUserInfo['team_vip_level'] || empty($aUserInfo['vip_level']))))) {
            $log['res'] = $upgradeRes;
        ...
        $allowUpgrade = TeamMemberVdc::where(['status' => 1, 'level' => $aUserLevel])->value('close_upgrade');
        ...
        $aNextLevel = TeamMemberVdc::where(['status' => 1, 'level' => ($aUserLevel - 1)])->order('level desc')->findOrEmpty()->toArray();
        ...
        //判断是否需要判断直推下级中排名第三的用户的销售额(小区销售额,小区指直推下级中排名第三的用户的销售额)
        if (doubleval($aNextLevel['village_performance']) > 0) {
            $dupMap[] = ['link_superior_user', '=', $uid];
            $dupMap[] = ['status', '=', 1];
            $dupMap[] = ['level', '>', 0];
            $directTeamUserPfm = TeamMemberModel::where($dupMap)->column('team_sales_price', 'uid');
            if (count($directTeamUserPfm) >= 3) {
                arsort($directTeamUserPfm);
                $villageUser = array_slice($directTeamUserPfm, 2, 1);
                $villageUser = array_values($villageUser);
                $villageUserPfm = $villageUser[0] ?? 0;
                if ((string)$villageUserPfm < (string)$aNextLevel['village_performance']) {
                    $log['upgradeTopUser'][] = $this->memberTopUserUpgrade($aMemberInfo['link_superior_user'] ?? '');
                    $log['now_village_performance'] = $directTeamUserPfm;
                    return $this->recordError($log, ['msg' => '小区业绩为' . $villageUserPfm . '不满足下级小区业绩目标' . $aNextLevel['village_performance'] . ' ,无法升级!']);
                }
            }
        }
        ...
        if ($aNextLevel['need_team_condition'] == 1) {
            ...
            //老方案为只检查直推符合等级的人数,新方案为以直推为首以下的团队中(包含直推自己)有一个以上符合等级的就算一个
            $directRes = false;
...
//            }
        ...
        if (!empty($directRes) && !empty($allRes) && !empty($directMemberRes)) {
            ...
            $upgradeRes = Db::transaction(function () use ($aNextLevel, $uid) {
                $userSave['team_vip_level'] = $aNextLevel['level'];
                ...
                $member = TeamMemberModel::update($memberSave, ['uid' => $uid]);

                //修改分润第一人冗余结构
                //先修改自己的,然后修改自己整个团队下级的
//                $dRes = $this->refreshDivideChain(['uid' => $uid]);
//                //团队的用队列形式,避免等待时间太长
//                $dtRes = Queue::later(10, 'app\lib\job\TeamMemberChain', ['searUser' => $uid, 'handleType' => 1], config('system.queueAbbr') . 'TeamMemberChain');
...
//        $this->log($log, 'info');
        ...
        if (!empty($log['upgradeRes'] ?? false)) {
            $upgradeQueue = Queue::push('app\lib\job\TeamMemberUpgrade', ['uid' => $uid], config('system.queueAbbr') . 'TeamMemberUpgrade');
        }
    ...
    public function memberUpgradeOld(string $uid, bool $topUserUpgrade = true)
    {
        ...
        $member = (new TeamMemberModel());
        $divide = (new TeamDivide());
        $aMemberInfo = $member->where(['uid' => $uid, 'status' => 1])->field('uid,user_phone,member_card,level,type,link_superior_user')->findOrEmpty()->toArray();
        $aUserLevel = $aMemberInfo['level'] ?? 0;
        ...
        if (!empty($checkOrderTeam['allUser']['onlyUidList'])) {
            $orderTotalPrice = (new TeamMemberModel())->getUserTeamOrderPrice($checkOrderTeam['allUser']['onlyUidList'], [1, 2]);
    ...
    public function becomeMember(array $orderInfo)
    {
        ...
        if (!empty($orderInfo)) {
            ...
            $memberVdc = TeamMemberVdc::where(['status' => 1])->order('level asc')->select()->toArray();
            $memberFirstLevel = max(array_column($memberVdc,'level'));
            if(empty($mUser['vip_level'] ?? 0)){
                $res['res'] = false;
                $res['msg'] = '非商城会员无法成为团队业绩的会员';
            ...
            if (!empty($mUser)) {
                if (intval($mUser['team_vip_level']) > 0) {
                    $res['res'] = false;
                    $res['msg'] = $orderInfo['uid'] . '该用户已经为' . ($levelName[$mUser['team_vip_level']] ?? " <未知等级> ") . ',无法继续绑定升级';
                    $this->recordError($log, ['msg' => $res['msg']]);
                    return $res;
                }
                ...
                //判断销售额
                $userTotalSalePrice = $mUser['team_sales_price'] ?? 0;
//                $userTotalSalePrice = 0;
//                //统计该用户的销售额
//                $sMap[] = ['a.link_uid', '=', $orderInfo['uid']];
//                $sMap[] = ['a.type', '=', 1];
//                $sMap[] = ['a.status', '=', 1];
//                $sMap[] = ['a.record_team', '=', 1];
//                $sMap[] = ['a.record_status', '=', 1];
//
//                $userTotalSalePriceList = TeamPerformance::alias('a')
//                    ->join('sp_order_goods b','a.order_sn = b.order_sn','left')
//                    ->where(function($query){
//                        $whereOr1[] = ['', 'exp', Db::raw('b.real_pay_price - b.refund_price > 0 and b.activity_sign in (' . implode(',', $this->activitySign). ')')];
//                        $whereOr2[] = ['', 'exp', Db::raw('b.crowd_code is not null and b.crowd_round_number is not null and b.crowd_period_number is not null')];
...
//                    $log['allTeam'] = $allTeam;
                ...
                $userMember = TeamMemberModel::where(['uid' => $orderInfo['uid'], 'status' => [1, 2]])->findOrEmpty()->toArray();
                $userShopMemberInfo = Member::where(['uid' => $orderInfo['uid'], 'status' => [1, 2]])->findOrEmpty()->toArray();

                $log['userMember'] = $userMember;
                $log['userShopMemberInfo'] = $userShopMemberInfo;
                if (empty($userShopMemberInfo)) {
                    $res['res'] = false;
                    $res['msg'] = $orderInfo['uid'] . '该用户非商城会员, 无法继续绑定升级';
                    return $this->recordError($log, ['msg' => $res['msg']]);
                }
...
//                            }
//                        }
                        $update['team_chain'] = $userShopMemberInfo['team_chain'] ?? null;
                        $update['divide_chain'] = $userShopMemberInfo['divide_chain'] ?? null;
                        $update['upgrade_time'] = time();
                        $dbRes = TeamMemberModel::update($update, ['uid' => $orderInfo['uid']]);
//                        //修改分润第一人冗余结构
//                        Queue::push('app\lib\job\TeamMemberChain', ['uid' => $orderInfo['uid'], 'handleType' => 1], config('system.queueAbbr') . 'TeamMemberChain');
...
//                            //尝试重新找一次上级是否有团队会员
//                            $topUser = TeamMemberModel::where(['uid' => $topUser])->findOrEmpty()->toArray();
//                            if (!empty($topUser ?? [])) {
...
//                        }
//                    }
                    $dbRes = TeamMemberModel::create($update);

//                    //修改分润第一人冗余结构
//                    Queue::push('app\lib\job\TeamMemberChain', ['uid' => $update['uid'], 'handleType' => 1], config('system.queueAbbr') . 'TeamMemberChain');
                }

                $userRes = User::update(['team_vip_level' => $update['level']], ['uid' => $mUser['uid']]);
    ...
    public function becomeMemberOld(array $orderInfo)
    {
        ...
        if (!empty($orderInfo) && ($orderInfo['order_type'] == 3) && !empty($orderInfo['link_child_team_code']) && !empty($orderInfo['link_superior_user'])) {
            ...
            $orderTopUser = TeamMemberModel::where(['uid' => $orderInfo['link_superior_user']])->findOrEmpty()->toArray();

            $log['topUserInfo'] = $orderTopUser ?? [];
            if (!empty($mUser)) {
                if (intval($mUser['team_vip_level']) > 0) {
                    $res['res'] = false;
                    $res['msg'] = $orderInfo['uid'] . '该用户已经为' . $levelName[$mUser['team_vip_level']] . ',无法继续绑定升级';
                    $this->recordError($log, ['msg' => $res['msg']]);
                    return $res;
                }
                //如果原先作为普通会员但是有推荐过人,存在会员体系中的话,允许重新跟上级要求购买团长大礼包然后重新修改会员体系中的登记信息
                ...
                $userRes = User::update(['team_vip_level' => $update['level'], 'is_new_vip' => 2, 'link_superior_user' => $update['link_superior_user']], ['uid' => $mUser['uid']]);
    ...
    public function assignUserLevel(array $data)
    {
        ...
        if(!empty($topUser)){
            $this->recordError($log, ['msg' => '团队代理人不允许指定上级用户, 仅允许修改等级']);
            throw new MemberException(['msg' => '团队代理人不允许指定上级用户, 仅允许修改等级']);
        }

        $userInfo = User::with(['teamMember'])->where(['uid' => $uid, 'status' => 1])->findOrEmpty()->toArray();
        if (!empty($topUser)) {
            $topUserInfo = User::with(['teamMember'])->where(['uid' => $topUser, 'status' => 1])->findOrEmpty()->toArray();
        }
        $userMemberInfo = [];
        //用户会员信息
        $userMemberInfo = Member::where(['uid' => $uid, 'status' => 1])->findOrEmpty()->toArray();
        if (empty($userMemberInfo)) {
            throw new MemberException(['msg' => '非会员无法升级为团队代理人']);
        }
        ...
        if (!empty($this->canNotOPERUidList)) {
            ...
            //团队代理人冗余团队结构完全依赖会员, 不重新判断新的团队结构
//        //判断是否存在头尾相接的情况,如果需要指定的上级的团队结构包含当前用户,则不允许指定
//        if (!empty($userInfo['teamMember']['link_superior_user'] ?? null) && $topUser != ($userInfo['teamMember']['link_superior_user'])) {
//            if (!empty($topUserInfo['teamMember']['team_chain'] ?? null) && (strpos($topUserInfo['teamMember']['team_chain'], $uid) !== false)) {
...
//        }
        ...
        if (empty($userInfo['teamMember']) && empty($userInfo['team_vip_level'])) {
            $uAssign['team_vip_level'] = $level;
        ...
        $mAssign['team_chain'] = $userMemberInfo['team_chain'];
        $mAssign['divide_chain'] = $userMemberInfo['divide_chain'];

        if (!empty($mAssign['link_superior_user'] ?? null)) {
            $topUserInfo = User::with(['teamMember'])->where(['uid' => $mAssign['link_superior_user'], 'status' => 1])->findOrEmpty()->toArray();
        }
        $log['topUserInfo'] = $topUserInfo ?? [];
        ...
        $memberInfo = TeamMemberModel::where(['uid'=>$uid,'status'=>1])->findOrEmpty()->toArray();
        if (empty(doubleval($memberInfo['team_sales_price'] ?? null))) {
            $memberInfo['team_sales_price'] = $userInfo['team_sales_price'];
            $mAssign['team_sales_price'] = $userInfo['team_sales_price'];
        }
        //计算销售额偏移量
        $memberGrowthValue = TeamMemberVdc::where(['status' => 1])->column('sales_price', 'level');
        ...
        $dbRes = Db::transaction(function () use ($uAssign, $mAssign, $uid, $topUser, $topUserInc, $topUserDes, $needChangeTeam, $userInfo, $topUserInfo, $changeType, $needChangeDivideChain,$memberGrowthValue,$newMember,$memberInfo) {
            if (!empty($uAssign)) {
            ...
            if (!empty($mAssign)) {
                ...
                if (!empty($mAssign['level']) && ($mAssign['level'] != $userInfo['team_vip_level'])) {
                    $mAssign['upgrade_time'] = time();
                    $demotionGrowthValue = TeamMemberVdc::where(['status' => 1, 'level' => $mAssign['level']])->value('demotion_sales_price');
                    $mAssign['demotion_team_sales_price'] = $demotionGrowthValue ?? $memberGrowthValue[$userInfo['team_vip_level']];
...
//                    //累加普通用户的原有成长值
//                    if (empty($userInfo['team_vip_level']) && !empty(doubleval($userInfo['growth_value']))) {
//                        $mAssign['growth_value'] += $userInfo['growth_value'] ?? 0;
//                    }
                    $mRes = TeamMemberModel::create($mAssign);
                } else {
                    $mRes = TeamMemberModel::update($mAssign, ['uid' => $uid]);
                }
            }
            if (!empty($uAssign)) {
                //记录操作日志
                $mLog['uid'] = $uid;
                $mLog['before_level'] = $userInfo['team_vip_level'] ?? 0;
                $mLog['after_level'] = $uAssign['team_vip_level'];
                ...
                (new AdminLog())->teamMemberOPER($mLog);
            }
...
//                $userAllTeam = $divideService->getTeamAllUserGroupByLevel($memberInfo['child_team_code']);
                ...
                if (!empty($dirUserCount)) {
                    ...
                    $updateTime = time();
                    if (!empty($nowTeamChain)) {
                        $changeTeamRes = Db::query("UPDATE sp_team_member set team_chain=REPLACE(team_chain,'" . $oldTeamChain . "','" . $nowTeamChain . "') , update_time = '" . $updateTime . "' where uid in (select a.* from (SELECT uid from sp_team_member where FIND_IN_SET('" . $uid . "',team_chain) and status in (1,2)) a)");
                    }

                    //修改分润第一人冗余结构
                    //团队的用队列形式,避免等待时间太长
                    $dtRes = Queue::push('app\lib\job\TeamMemberChain', ['searUser' => $parentMember['uid'], 'update_time' => $updateTime, 'handleType' => 1], config('system.queueAbbr') . 'TeamMemberChain');
                }
...
//                }
            ...
            //修改当前用户信息<删除会员信息和等级>
            $res = TeamMemberModel::update(['status' => -1, 'growth_value' => 0, 'team_chain' => null, 'divide_chain' => null], ['uid' => $uid, 'status' => [1, 2]]);
            User::update(['team_vip_level' => 0, 'team_number' => 0, 'fronzen_balance' => 0, 'growth_value' => 0], ['uid' => $uid, 'status' => [1, 2]]);
...
//        }
        ...
        $list = TeamMemberModel::where($map)->field('uid,level,team_chain')->select()->toArray();
        if (empty($list)) {
            //记录日志
            (new Log())->setChannel('memberChain')->record($log);
            return false;
        ...
        if (!empty($TopUser)) {
            ...
            if (!empty($findTopUser)) {
                $topUserList = TeamMemberModel::where(['uid' => $findTopUser, 'status' => 1])->column('level', 'uid');
...
//                $sql =  "UPDATE sp_team_member set divide_chain = CASE uid";
//                foreach ($firstNumber as $key => $value) {
////                    $sql = "UPDATE sp_team_member set divide_chain = '". json_encode($value,256)."'"." where uid = '".$key."';";
            ...
            if (!empty($firstTeamNumber)) {
                ...
                if (!empty($firstTeamNumber)) {
                    foreach ($firstTeamNumber as $key => $value) {
                        ...
                        $sql[$key] = "UPDATE sp_team_member set divide_chain = ( CASE team_chain";
                        foreach ($value as $k => $v) {
...
//                    $sql .= sprintf("WHEN %d THEN %d ", $key, json_encode($value,256));
    ...
    {
        //用户信息
        $userInfo = TeamMemberModel::where(['uid' => $data['uid']])->field('uid,user_phone,level,team_sales_price,team_sales_price_offset,demotion_team_sales_price')->findOrEmpty()->toArray();
        //判断用户等级是否需要降级
        ...
        if ($userNowLevel != $userInfo['level']) {
            ...
            $this->log(['msg' => '用户' . $data['uid'] . '销售额不足,需要降级 ' . '现在等级为 ' . $userInfo['level'] . '修改为' . $userNowLevel, 'userInfo' => $userInfo], 'error');
            User::where(['uid' => $userInfo['uid']])->save(['team_vip_level' => $userNowLevel]);
            TeamMemberModel::where(['uid' => $userInfo['uid']])->save(['level' => $userNowLevel, 'demotion_team_sales_price' => $demotionGrowthValue]);

            //修改分润第一人冗余结构
            //先修改自己的,然后修改自己整个团队下级的
            $dRes = $this->refreshDivideChain(['uid' => $userInfo['uid']]);
            //团队的用队列形式,避免等待时间太长
            $dtRes = Queue::later(10, 'app\lib\job\TeamMemberChain', ['searUser' => $userInfo['uid'], 'handleType' => 1], config('system.queueAbbr') . 'TeamMemberChain');
        }
        return true;
    }
        ...
        switch ($memberVdc['demotion_type']) {
            ...
            case 2:
                ...
                if ($checkLevel == $userInfo['level']) {
                    $judgeGrowthValue = $userInfo['demotion_team_sales_price'] ?? ($memberVdc['demotion_sales_price'] ?? $memberVdc['sales_price']);
                } else {
                    $judgeGrowthValue = $memberVdc['demotion_sales_price'] ?? $memberVdc['sales_price'];
                }
