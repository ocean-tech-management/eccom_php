...
class Ppyl extends BaseValidate
{
    public $errorCode = 500119;
...
//        'join_number.egt' => '每个人参加此团的次数必须为一次以上',
    ...
    protected $scene = [
        'create' => ['activity_title', 'activity_cover', 'expire_time', 'type', 'start_user_type', 'join_user_type', 'start_time', 'end_time', 'group_number'],
        'edit' => ['activity_code', 'activity_title', 'activity_cover', 'expire_time', 'type', 'start_user_type', 'join_user_type', 'start_time', 'end_time', 'group_number'],
    ...
    {
        $exist = (new PpylActivity())->where([$fieldName => trim($value), $this->statusField => [1, 2]])->count();
    ...
    public function checkNameE($value, $rule, $data, $fieldName)
    {
        ...
        $exist = (new PpylActivity())->where($map)->where([$fieldName => trim($value), $this->statusField => [1, 2]])->count();
        return empty($exist) ? true : '该活动名称已存在';
    }
}